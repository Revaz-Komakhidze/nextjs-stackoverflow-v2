      ...
      {sidebarLinks.map((item) => {
        ...
        return (
          <SheetClose key={item.route}>
            <Link
              href={item.route}
              className={`${isActive ? "primary-gradient rounded-lg text-light-900" : "text-dark300_light900"} flex items-center justify-start gap-4 bg-transparent py-4`}
            >
              <Image
                src={item.imgURL}
                alt={item.label}
    ...
    <Sheet>
      ...
      >
        <Link href={"/"} className="flex items-center gap-2">
          ...
          />
          <p className="h2-bold  text-dark100_light900 dark:text-light-900 max-sm:hidden">
